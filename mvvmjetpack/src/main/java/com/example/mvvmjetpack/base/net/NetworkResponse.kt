package com.example.mvvmjetpack.base.net
...
fun <T : Any> NetworkResponse<T>.exceptionOrNull(): Throwable? = when (this) {
...
fun <T : Any> NetworkResponse<T>.getOrThrow(): T = when (this) {
    is NetworkResponse.Success -> data
...
fun <T : Any, R : Any> NetworkResponse<T>.toResult(mapper: ((T) -> R)): Result<R> {
    return if (this is NetworkResponse.Success<T>) {
